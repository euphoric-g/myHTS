...
public:
	...
	std::unordered_map<CS, CS> GetLoginInfoAll() {
		std::unordered_map<CS, CS> ret;
		std::vector<CS> strlst = { _T("ACCOUNT_CNT"), _T("ACCNO"), _T("USER_ID"), _T("USER_NAME"),
		_T("KEY_BSECGB"), _T("FIREW_SECGB"), _T("GetServerGubun") };
		for (auto it = strlst.begin(); it != strlst.end(); it++) {
			ret[*it] = GetLoginInfo(*it);
		}
		ret[_T("STATUS")] = _T("GOOD");
		return ret;
	}
